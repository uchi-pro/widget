<template>
  <div v-if="coursesFetched" class="uchi">
    ...
    <div class="uchi-cart" :class="{ 'uchi-cart_opened' : isCartOpened() }">
        <a @click="toggleCart()" class="uchi-cart__toggle" :title=" isCartOpened() ? 'Закрыть корзину' : 'Открыть корзину' ">
            <span>Корзина</span>
        </a>
        ...
        <div class="uchi-cart__content">
            ...
            <div v-if="inCartCoursesCount > 0">
                ...
                <ul>
                    <li><router-link :to="{ name: 'cart' }">Перейти в корзину</router-link></li>
                </ul>
            </div>
            <div v-else>
                Корзина пуста
